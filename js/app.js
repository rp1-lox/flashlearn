...
(function () {
  ...
  function askQuestion(opts) {
    ...
    function submit(dontKnow) {
      ...
      // Like Quizlet: after a miss, a "Don't know", or a misspelling, type the
      // correct answer once before moving on. A misspelling still counts as right.
      var canRetype = opts.deck.settings.retype !== false && String(card[sd.a] || '').trim() !== '';
      if (g.correct && !(g.typo && canRetype)) {
      ...
      var head = g.correct
        ? '<div class="fb good">Correct, but check the spelling ✓</div>'
        : '<div class="fb bad">' + (dontKnow ? 'No problem — study this one.' : g.close ? 'Close, but not quite.' : 'Not quite.') + '</div>';
      var result = !!g.correct;
      $('#fb').innerHTML = head +
        (dontKnow ? '' : '<div class="small muted">You wrote</div><div class="yours">' + esc(given) + '</div>') + ans +
        (canRetype ? '<form id="rform" autocomplete="off"><label class="small muted" for="rans">Type the correct answer to continue</label>' +
        ...
        '<div class="row gap wrap">' + (dontKnow || g.correct ? '' : '<button class="btn" id="override">I was right</button>') + '<div class="grow"></div>' +
        (canRetype ? '<button class="btn primary" id="rcheck" type="button">Check <span class="kbd">Enter</span></button>' : continueBtn(true)) + '</div>';
      on('#override', 'click', function () { finish(true); });
      revealFeedback();
      if (!canRetype) { wireContinue(function () { finish(result); }); return; }
      var rin = $('#rans');
      ...
      function check() {
        ...
        // The copy must be spelled right: exact after normalization, no typo allowance.
        var copy = FL.grade(v, card[sd.a]);
        if (copy.correct && !copy.typo) {
          rin.readOnly = true;
          $('#rmsg').innerHTML = '<span class="good-text">Got it ✓</span>';
          later(function () { finish(result); }, 500);
        } else {
          $('#rmsg').textContent = copy.correct ? 'Almost. Check the spelling against the answer above.' : 'Not quite. Copy the answer above.';
          rin.select();
        }
      }
